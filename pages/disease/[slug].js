...
import { diseaseUrl } from "../../utils/endpoints";
...
import { getDisease } from "@/utils/powerFunctions";
...
function DiseasePage(props) {
    ...
    let videoWidgets;
    const relatedArticle = props?.diseaseData?.read_this_later || [];
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoadingLast(true);
                const res = await getDisease(slug, router?.locale);
    ...
    const onSubmit = async (e, id) => {
        if (id) {
            ...
            try {
                let res = await APIV3.post("/review-disease", payload);
    ...
    return (
        <>
            <MetaDataCustom metaData={props?.diseaseData} />
            {loadingLast ? (
                ...
                <section className="articleInnerPage section_fit_screen articleDisease revampDisease">
                    <div className="banner_diabities">
                                ...
                                <Row>
                                    <Col lg="12">
                                        <Breadcrumb className="breadCrumb">
                                            <Breadcrumb.Item href="/sehat-a-z">Diseases
                                            ...
                                            <Breadcrumb.Item href={props?.diseaseData?.redirect_url}>
                                                {props?.diseaseData?.name}
                                            </Breadcrumb.Item>
                                        </Breadcrumb>
                                    </Col>
                                </Row>
                        ...
                        <div className="breadCrumbMainBox hk_banner_area" style={{ backgroundImage: `url(${props?.diseaseData?.cover_image})`, height: '720px', backgroundSize: 'cover', backgroundRepeat: 'no-repeat', position: 'relative', backgroundPosition: '100% 100%' }}>
...
export async function getServerSideProps(context) {
    ...
    const { slug } = context.query; // Extract the slug from the query parameters

    const diseasePage = `${diseaseUrl}${slug}`;

    try {
        const diseasePageContent = await APIV3.get(diseasePage, {
            headers: {
                platform: "web",
                locale: apiLocale,
            },
        ...
        const diseasePageData = diseasePageContent?.data?.data;

        if (diseasePageContent?.status == 200) {
            // Fix the variable name here
            return {
                props: {
                    diseaseData: diseasePageData,
