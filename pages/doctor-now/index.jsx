...
import { getDoctorNowWidgets } from "@/utils/endpoints";
...
export default function index(props) {
  console.log(props, "doctor Page");

  const { doctorNowWidgets, _nextI18Next } = props;
  ...
  useEffect(() => {
    setPageName(doctorNowWidgets?.slug);
  }, [doctorNowWidgets])

  return (
    <WraperDoctor>
      <MetaDataCustom metaData={doctorNowWidgets} />
      <section className={"doctorNowPage"}>
        <Suspense fallback={<Loader />}>
          {doctorNowWidgets?.widgets?.map((item, index) => {
            const { key_type } = item;
...
export const getServerSideProps = async (context) => {
  ...
  try {
    ...
    response = await APIV3.get(getDoctorNowWidgets, {
      headers: {
        platform: "web",
        locale: apiLocale,
      },
    ...
    let doctorNowWidgets = response?.data?.data;
    return {
      props: {
        doctorNowWidgets,
