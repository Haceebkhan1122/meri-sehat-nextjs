...
import styles from './programIncludes.module.scss';
import Router, { useRouter } from 'next/router';

const ProgramIncludes = (props) => {
    const isMobile = useMediaQuery('(max-width:768px)');
    const { widgetData } = props;

    return (
        <section className={`${styles.programIncludes} programIncludes`} style={{backgroundColor : props?.widgetData?.data?.[0]?.card_1_color }}>
                            ...
                            <Col lg={6} className={`${styles.wrap_ll_con1} my-auto ms-auto`}>
                                <div className={`${styles.left__sec} left__sec`}>
                                    ...
                                    <div className={styles.btnWrape}>
                                        <button className='buttonWithBgColor_hover' onClick={() => Router.push(`${ props?.widgetData?.redirect_url}`)}> {props?.widgetData?.data?.[0]?.button_text} </button>
                                    </div>
                                </div>
                            </Col>
