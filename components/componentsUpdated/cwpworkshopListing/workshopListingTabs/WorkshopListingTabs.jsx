...
import styles from "./workshopListingTabs.module.scss"
...
function workshopListingTabs({ setAllWorkshopListener, selectedCategoryId, setCurrentPage, allWorkshopListener, setSearch, setSelectedCategoryId, setSelectedCategory, selectedCategory, workShops, handlePageClick, currentPage, totalPages }) {
    const router = useRouter();

    useEffect(() => {
        if (router.query.cat && router.query.cat.trim().toLowerCase() === "all") {
            setSelectedCategory("all-workshops");
        } else {
            const filteredCategory = workShops?.categories.find(
                (category) => category?.id == selectedCategoryId
            );
            setSelectedCategory(filteredCategory?.title || "");
        }
    }, [workShops?.categories, selectedCategory, router.query]);
    ...
    const handleCategoryChangeAllWorks = () => {
        setCurrentPage("1")
    ...
    return (
        <>
            <section className={`${styles.tabsSection} workshopWellnesstabsSection`}>
                <Container>
                    <Row>
                        <Col lg={12}>
                            <Tab.Container id="left-tabs-example" defaultActiveKey="first">
                                <Row>
                                    <Col md={12}>
                                        <Nav
                                            variant="pills"
                                            className={`${styles.flexColumn} breadcrumbWorkshop`}
                                        >
                                            <Nav.Item className={`${styles.navItem}`} onClick={(e) => handleCategoryChangeAllWorks(e)}>
                                                <Nav.Link
                                                    className={`${styles.button} ${selectedCategory == "all-workshops" ? styles.active : ''}`}
                                                    eventKey={'all'}
                                                >
                                                    All Workshops
                                                </Nav.Link>
                                            </Nav.Item>
                                            {workShops?.categories?.length > 0 &&
                                                workShops?.categories?.map((category) => (
                                                    <React.Fragment key={category?.id}>
                                                        {category?.status === true && (
                                                            <Nav.Item className={`${styles.navItem}`} onClick={() => handleCategoryChange(category)}>
                                                                <Nav.Link
                                    ...
                                    <Col md={12}>
                                        <Tab.Content>
                                            <Row className={`${styles.secondRowImages}`}>
                                                {workShops?.workshops?.data?.length > 0 && workShops.workshops.data?.map((workshop) => (
                                                    <Col lg={4} className={`${styles.fiveFRem}`}>
                                                        <Link href={`/corporate-wellness-workshop-detail/${workshop?.id}`}>
                                                            <Image
                                                ...
                                                {!workShops?.workshops?.data?.length > 0 ? (
                                                    <>
                                                        <div className='text-center hk_workshop_not_found mb-5'>
                                                            No workshop found
                                                        </div>
                                                    </>
                                                ) : null}
                        ...
                        <Col md={12} className='pagination_workshop'>
                            {totalPages > 1 ? (
                                <>
                                    <ReactPaginate
...
export default workshopListingTabs